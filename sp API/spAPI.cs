...
namespace SP_api
{
    public class SPapi
    {
        ...
        public async Task<string> GetPaymentLink(int amount, string webhookUrl, string data, string redirectUrl = "https://spworlds.ru")
        {
            ...
            var jsonAnswer = await client.PostAsync("https://spworlds.ru/api/public/payment", paymentRequest).Result.Content.ReadAsStringAsync();
        ...
        {
            int balance = -1;
            var jsonAnswer = await client.GetAsync(baseURL + "card").Result.Content.ReadAsStringAsync();
        ...
        public async Task<string> GetUsername(string discordId)
        {
            var jsonAnswer = client.GetAsync(baseURL + "users/" + discordId).Result;
            var username = JsonConvert.DeserializeObject<UsernmeAnswer>(await jsonAnswer.Content.ReadAsStringAsync()).username;
        ...
        public async Task<string> Pay(string receiver, int amount, string comment)
        {
            ...
            var status = await client.PostAsync(baseURL + "transactions", paymentRequest);
            return status.StatusCode.ToString() == "OK" ? "Успешно!" : "Ошибка! Возможно такого номера карты нет.";
        }
